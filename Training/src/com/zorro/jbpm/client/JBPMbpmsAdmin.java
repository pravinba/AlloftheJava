...
public class JBPMbpmsAdmin {
	...
	public static void main(String[] args) {
		...
		parameter.put("in_emp_leave_days",150);
		parameter.put("in_emp_leave_type","Maternity");
		parameter.put("in_emp_name","Pravin");		
		parameter.put("in_emp_no",111);
		...
		List<Status> status = getTaskStatusList();
		 ...
		 List<TaskSummary> summary = taskService.
				 getTasksByStatusByProcessInstanceId(new Long(2), status, "bpmsAdmin");
		 ...
		 for(TaskSummary task:summary){
			 ...
			 if(task.getStatus().equals(Status.InProgress)){
				   ...
				   taskService.complete(task.getId(), "bpmsAdmin", parameter);
			 }
		 }
